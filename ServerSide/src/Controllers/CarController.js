...
// Function to remove non-vehicle items
const removeNonVehicleItems = async () => {
    try {
        await carDetails.deleteMany({
            type: { $in: ['Helmet', 'Gloves', 'Jacket'] }
        });
        console.log('Successfully removed non-vehicle items');
    } catch (error) {
        console.error('Error removing non-vehicle items:', error);
    }
};

// Call removeNonVehicleItems when the server starts
removeNonVehicleItems();
...
const getCars = async(req,res)=>{
    try {
        if(req.headers.authorization){
            // Only fetch vehicle items, excluding non-vehicle items
            const readData = await carDetails.find({
                $and: [
                    // Exclude items with these keywords in the name
                    { 
                        name: { 
                            $not: {
                                $regex: /(helmet|gloves|jacket|MM|sajal gorain|nexon)/i
                            }
                        }
                    },
                    // Only include items with valid vehicle types
                    {
                        type: {
                            $in: ['Car', 'SUV', 'Luxury', 'Sedan', 'Hatchback', 'MUV']
                        }
                    }
                ]
            });
